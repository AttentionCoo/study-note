...
| 序号 | 章节 | 核心内容 |
|------|------|----------|
| 一 | [方法](#一方法) | 邻接表、树链剖分、LCA、线段树、数论、快速幂、大数运算、乘法逆元、素数筛 |
| 二 | [千万注意要审题](#二千万注意要审题) | 审题要点、时间分配 |
| 三 | [经验](#三经验) | 数据范围与算法选择、巧设数组 |
| 四 | [动态规划](#四动态规划) | 字符串DP、最优子结构、无后效性、记忆化搜索、升维 |
| 五 | [解题思想](#五解题思想) | 自然思想、逆向思维、递归、DFS、二分 |
| 六 | [线段树想法](#六线段树想法) | 懒标记、下传机制、query下传 |
| 七 | [语法规则](#七语法规则) | typedef、字典、set、排序、vector、cstring、状态压缩 |
| 八 | [解题技巧](#八解题技巧) | 区间、图、二进制、状态压缩、记忆化搜索、双指针、BFS |
...
### 6.6.2 懒信息只下发一层，并且是该节点所包含的部分区间有修改才下传，否则只修改该节点，不改懒标记
...
### 8.3.2 如何在二进制中减去最后一个1

直接进行 `(x-1) & x` 操作

### 8.3.3 自己在二进制运算中要懂得构建特殊二进制串
