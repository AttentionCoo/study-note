...
| ------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **1. Nginx 路由层** | 请求发往 `/api/code/assist` $\rightarrow$ 命中隐藏配置中的 Java 代理 $\rightarrow$ 转发至 **Java (8080 端口)** | 请求发往 `/api/code/assist` $\rightarrow$ 命中动态代理 $\rightarrow$ 转发至 **Python (8000 端口)** 的 `/model/code/assist` |
| **2. 数据解析层**   | Java 后端使用 Jackson 框架尝试以标准 JSON 解析 Python 吐出的 `data: {...}` 流，引发 **Token错误** 崩溃。 | 请求直达 Python 后端，原生支持 SSE（Server-Sent Events）流式数据透传，前端完美实现打字机效果。 |
